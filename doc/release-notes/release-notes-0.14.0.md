Slemanium Core version 0.14.0 is now available from:

  <https://slemanium.org/bin/slemanium-core-0.14.0/>
...
Slemanium Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/slemanium/slemanium/blob/master/doc/zmq.md) for
...
When Slemanium Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `slemanium-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`slemanium-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/slemanium-cli -named help command="help"
    src/slemanium-cli -named getblockhash height=0
    src/slemanium-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/slemanium-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://slemanium.org/en/alert/2016-11-01-alert-retirement) in Slemanium Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Slemanium Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/slemanium/slemanium/pull/8753)
...
- #8722 `bb843ad` slemanium-cli: More detailed error reporting (laanwj)
...
- #7562 `1eef038` Bump transaction version default to 2 (slndrak)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnomsln)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnomsln)
...
- #8830 `ef0801b` test: Add option to run slemanium-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to slemanium-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to slemanium-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and slemanium-util-test.py (jnewbery)
...
- #9206 `e662d28` Make test constant consistent with consensus.h (slndrak)
...
- #9691 `fc67cd2` Init ECC context for `test_slemanium_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change slemanium address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` slemaniumd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow slemanium-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` slemanium-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to slemanium-tx (jnewbery)
