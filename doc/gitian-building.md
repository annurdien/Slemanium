...
*Setup instructions for a Gitian build of Slemanium Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Slemanium
...
- [Building Slemanium Core](#building-slemanium-core)
...
- [LXC](https://linuxcontainers.org/), see also [Gitian host docker container](https://github.com/gdm85/tenku/tree/master/docker/gitian-slemanium-host/README.md).
...
Clone the git repositories for slemanium and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/slemanium/slemanium
...
in the slemanium repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Slemanium Core
----------------

To build Slemanium Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the slemanium repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/slemanium/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/slemanium/slemanium
...
For example:
```bash
URL=https://github.com/laanwj/slemanium.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit slemanium=${COMMIT} --url slemanium=${URL} ../slemanium/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit slemanium=${COMMIT} --url slemanium=${URL} ../slemanium/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit slemanium=${COMMIT} --url slemanium=${URL} ../slemanium/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the slemanium git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../slemanium/contrib/gitian-descriptors/*|sort|uniq )
...
SLNPATH=/some/root/path/slemanium
SIGPATH=/some/root/path/slemanium-detached-sigs

./bin/gbuild --url slemanium=${SLNPATH},signature=${SIGPATH} ../slemanium/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/slemanium-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/slemanium-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/slemanium-osx-build.assert
```
...
[slemanium-core/gitian.sigs](https://github.com/slemanium-core/gitian.sigs/) repository, or if that's not possible create a pull
