...
The RPM spec file provided here is for Slemanium-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://slemanium.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Slemanium git repository](https://github.com/slemanium/slemanium) but are not part of
...
Currently only `Source10` is used, for the example `slemanium.conf` file.
...
in the slemanium git repository. At present only an SVG version of the Slemanium
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the slemanium
...
needed when 0.12.1 is released, a proper fix is already in the Slemanium git
master branch.
...
idea to build Slemanium Core against a static version of that library compiled
...
The `slemanium.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `slemanium-server` package. At this point, most RPM based
...
The `slemanium-server` package should have SELinux support. How to properly do
...
Slemanium-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/slemanium/). He has been
packaging Slemanium for Fedora at least since 2012.
...
4. I renamed the `slemanium` package that contains the Qt GUI to `slemanium-core` as
that appears to be how the general population refers to it, in contrast to
`slemanium-xt` or `slemanium-classic`. I wanted to make sure the general population
