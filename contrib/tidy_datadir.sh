#!/bin/bash
# Copyright (c) 2013 The Slemanium Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Slemanium database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Slemanium datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Slemanium datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Slemanium 0.7 datadir."
    ;;
  3)
    echo "Detected Slemanium pre-0.8 datadir."
    ;;
  4)
    echo "Detected Slemanium 0.8 datadir."
    ;;
esac
