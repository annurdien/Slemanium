...
###   slemaniumcore.org and slemanium.org and compares them.
...
WORKINGDIR="/tmp/slemanium_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://slemaniumcore.org"
HOST2="https://slemanium.org"
BASEDIR="/bin/"
VERSIONPREFIX="slemanium-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'slemanium-',
...
if [ $? -ne 0 ]; then
   echo "slemanium.org failed to provide signature file, but slemaniumcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "slemanium.org and slemaniumcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Slemanium Core binary release signing key installed?"
   fi
